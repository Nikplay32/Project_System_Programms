...
namespace RepairEquipment.Client.Services
{
    public sealed class EquipmentHistoryService : IEquipmentHistoryService
    {
        ...
        {
            var sql = @"SELECT ROW_NUMBER() OVER(ORDER BY eq.Brand ASC) AS ID, dd.ID as DocumentDetailsID, eq.Brand AS Brand,
                      eq.Model AS Model, et.Name AS EquipmentType, cl.Name AS Client, CONCAT(em.Name, em.Surname) AS Employee, 
                      do.DocumentNumber, dd.DocumentDateOut, dd.DocumentDateIn, dd.Quantity 
